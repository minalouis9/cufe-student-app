...
public class SemesterGPA extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AsyncCallWS_SemesterData semesterDetails = new AsyncCallWS_SemesterData();
        semesterDetails.execute();
    }

    private class AsyncCallWS_SemesterData extends AsyncTask<Void, Void, Void> {
        ...
        private void CalculateGPA()
        {
            for(int i=0;i<Count;i++)
            {
                if (StringGrades[i].equals("A+")|| StringGrades[i].equals("A"))
                    Grades[i] = 4.0;
                if (StringGrades[i].equals("A-"))
                    Grades[i] = 3.7;
                if (StringGrades[i].equals("B+"))
                    Grades[i] = 3.3;
                if (StringGrades[i].equals("B"))
                    Grades[i] = 3.0;
                if (StringGrades[i].equals("B-"))
                    Grades[i] = 2.7;
                if (StringGrades[i].equals("C+"))
                    Grades[i] = 2.3;
                if (StringGrades[i].equals("C"))
                    Grades[i] = 2.0;
                if (StringGrades[i].equals("C-"))
                    Grades[i] = 1.7;
                if (StringGrades[i].equals("D+"))
                    Grades[i] = 1.3;
                if (StringGrades[i].equals("D"))
                    Grades[i] = 1.0;
                if (StringGrades[i].equals("D-"))
                    Grades[i] = 0.7;
                if (StringGrades[i].equals("F"))
                    Grades[i] = 0.0;
            }
            double Mul;
